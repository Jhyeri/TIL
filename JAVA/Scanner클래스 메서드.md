...
그럼 버퍼에 \t가 남게 되는데, **nextLine()은 개행문자인 분리자를 포함**시키기 때문에 \t만 가져오고 프로그램이 종료된다. 
...
콘솔에서 키를 입력하고 **엔터키를 눌러야** 지금까지 쓴 내용이 버퍼에 전달되어 저장된다.
...
따라서 nextLine()을 실행한 후에는 아무 값도 남지 않게 된다.
...
반환문자는 필요 없으므로 메서드만 호출한 후, 새로 nextLine()을 실행해서 한 줄을 받아오면 된다.
